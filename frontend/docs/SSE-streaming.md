...
| `checking_history` | 멀티턴 히스토리 판단 중                                        |
| `routing_query`    | Intent Router + Query Rewriter + Filter Builder 수행 중 |
| `searching`        | Multi-Pool Hybrid Search 수행 중                        |
| `reranking`        | Cross-Encoder 재순위화 수행 중                              |
...
event: status
data: {"phase":"checking_history","message":"이전 대화와 연결된 질문인지 확인하고 있습니다."}

event: status
data: {"phase":"routing_query","message":"질문의 의도와 검색 조건을 분석하고 있습니다."}

event: status
data: {"phase":"searching","message":"관련 문서를 검색하고 있습니다."}

event: status
data: {"phase":"reranking","message":"검색된 문서의 관련도를 재정렬하고 있습니다."}
