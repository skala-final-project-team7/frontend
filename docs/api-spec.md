...
| ------ | ------------------------------------------------ |
| Method | `POST`                                           |
| URL    | `/api/v1/rag/query?stream=true`                  |
...
- `stream=true`: streaming 모드. SSE 스트림과 `status` 이벤트를 수신한다.
...
[프론트엔드]
  ...
  ├─ POST /api/conversations/{id}/chat            → BFF → POST /api/v1/rag/query?stream=true (SSE)
  │                                                 ├─ DB에서 이전 이력 조회 → ML 전달
  │                                                 └─ ML 응답을 DB 저장 + SSE 중계
